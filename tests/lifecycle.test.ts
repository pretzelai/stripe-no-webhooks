...
describe("Lifecycle: New Subscription", () => {
  ...
  test("grants multiple credit types independently", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "api_calls" })).toBe(100000);
    expect(await credits.getBalance({ userId: "user_1", creditType: "storage_gb" })).toBe(1000);
    expect(await credits.getBalance({ userId: "user_1", creditType: "seats" })).toBe(50);
  });
...
describe("Lifecycle: Upgrade Free → Paid", () => {
  test("revokes free credits and grants paid credits", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "storage_gb" })).toBe(100);
  });
  ...
  test("revokes only remaining free credits if some were consumed", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "api_calls" })).toBe(40);
...
describe("Lifecycle: Upgrade Paid → Paid", () => {
  test("keeps old credits and adds new credits (no proration compensation)", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "api_calls" })).toBe(700);
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "api_calls" })).toBe(10700);
  ...
  test("adds new credit types that did not exist in old plan", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "api_calls" })).toBe(1000 + 100000);
    expect(await credits.getBalance({ userId: "user_1", creditType: "storage_gb" })).toBe(1000);
    expect(await credits.getBalance({ userId: "user_1", creditType: "seats" })).toBe(50);
  });
});
...
describe("Lifecycle: Downgrade", () => {
  ...
  test("applies downgrade at renewal: revokes extra credit types, resets to new allocation", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "api_calls" })).toBe(7000);
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "storage_gb" })).toBe(0);
  });
  ...
  test("downgrade respects onRenewal: add mode", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "api_calls" })).toBe(7500);
  });
});
...
describe("Lifecycle: Renewal", () => {
  ...
  test("accumulates credits on renewal with onRenewal: add", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "storage_gb" })).toBe(50);
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "storage_gb" })).toBe(150);
  });
...
describe("Lifecycle: Cancellation", () => {
  test("revokes all credits on subscription cancellation", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "api_calls" })).toBe(6000);
    expect(await credits.getBalance({ userId: "user_1", creditType: "storage_gb" })).toBe(100);
...
describe("Lifecycle: Seat-Users Mode", () => {
  test("onSubscriptionCreated grants credits to first_seat_user_id if present", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_alice", creditType: "api_calls" })).toBe(1000);
    expect(await credits.getBalance({ userId: "org_1", creditType: "api_calls" })).toBe(0);
  });
  ...
  test("onSubscriptionRenewed grants to all active seat users", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_alice", creditType: "api_calls" })).toBe(1000);
    expect(await credits.getBalance({ userId: "user_bob", creditType: "api_calls" })).toBe(1000);
  });
  ...
  test("onSubscriptionCancelled revokes from all seat users", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_alice", creditType: "api_calls" })).toBe(0);
    expect(await credits.getBalance({ userId: "user_bob", creditType: "api_calls" })).toBe(0);
  });
...
describe("Lifecycle: Yearly Plans - New Subscription", () => {
  ...
  test("yearly subscription grants 12x for multiple credit types", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "api_calls" })).toBe(120000);
    expect(await credits.getBalance({ userId: "user_1", creditType: "storage_gb" })).toBe(1200);
  });
  ...
  test("weekly with non-divisible allocation rounds up", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "api_calls" })).toBe(25); // ceil(100/4)
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "api_calls" })).toBe(25); // ceil(99/4) = 25
  });
});
...
describe("Lifecycle: Yearly Plans - Renewal", () => {
  test("yearly renewal grants 12x credits with onRenewal: reset", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "api_calls" })).toBe(12000);
  });
  ...
  test("yearly renewal with onRenewal: add accumulates 12x", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "storage_gb" })).toBe(1700);
  ...
  test("weekly renewal grants 0.25x credits", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "api_calls" })).toBe(250);
  });
});
...
describe("Lifecycle: Yearly Plans - Same-Plan Interval Changes", () => {
  test("monthly to yearly upgrade: keeps old credits + grants 12x new", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "api_calls" })).toBe(12700);
  });
  ...
  test("yearly to monthly downgrade: scheduled, grants monthly at period end", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "api_calls" })).toBe(8000);
...
describe("Lifecycle: Yearly Plans - Cross-Plan Upgrades", () => {
  test("Basic Monthly to Pro Yearly: keeps old + grants 12x Pro", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "api_calls" })).toBe(600);
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "api_calls" })).toBe(120600);
...
describe("Lifecycle: Yearly Plans - Downgrades", () => {
  test("Pro Yearly to Basic Monthly downgrade: resets to monthly Basic allocation", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_1", creditType: "api_calls" })).toBe(70000);
...
describe("Lifecycle: Yearly Plans - Seat-Users Mode", () => {
  test("yearly subscription grants 12x to first seat user", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_alice", creditType: "api_calls" })).toBe(12000);
    expect(await credits.getBalance({ userId: "org_1", creditType: "api_calls" })).toBe(0);
  });
  ...
  test("yearly renewal grants 12x to all seat users", async () => {
    ...
    expect(await credits.getBalance({ userId: "user_alice", creditType: "api_calls" })).toBe(12000);
    expect(await credits.getBalance({ userId: "user_bob", creditType: "api_calls" })).toBe(12000);
  });
});
